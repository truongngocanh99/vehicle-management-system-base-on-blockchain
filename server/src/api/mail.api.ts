...
export const mail_registry_citizen = async(dataSend:any) => {
  ...
  const info = await transporter.sendMail({
    ...
    <p>Cảm ơn bạn đã đăng ký tài khoản trên hệ thống đăng ký xe trực tuyến. Để có được trải nghiệm dịch vụ và được hỗ trợ tốt nhất, bạn cần hoàn thiện xác thực tài khoản.</p>
    <p>Vui lòng bấm nút Xác thực để hoàn tất quá trình này.</p>
    <a href="http://localhost:8000" style="color:#fff;text-decoration:none;display:inline-block;background-color:#00b14f;padding:12px 20px;font-weight:bold;border-radius:4px" target="_blank"><strong>Xác thực</strong></a>
    `,
...
export const  mail_tranfer_car= async(dataSend:any) => {
  const info = await transporter.sendMail({
    ...
    <p>Chủ xe:  ${dataSend.currentOwner_name}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Địa chỉ: ${dataSend.current_district}, ${dataSend.current_city}</p>
    ...
    <p>Vui lòng đến trụ sở CSGT tại nơi đăng ký để hoàn tất thủ tục </p>
